...
def get_pretrained_emb(model, tokenizer, sentences, entity_pos, eid2idx, np_file, dim=768, batch_size=256):  # 768
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dataset', default='./wiki', help='path to dataset folder')
    parser.add_argument(
        '-vocab', default='entity2id-sentence.txt', help='vocab file')
    parser.add_argument(
        '-sent', default='sentences.json', help='sent file')
    parser.add_argument(
        '-npy_out', default='pretrained_emb-sentence.npy', help='name of output npy file')
    parser.add_argument('-entity_pos_out', default='entity_pos-sentence.pkl',
                        help='name of output entity index file')
    ...
    # print(eid2idx)
    sentences, entity_pos = get_masked_sentences(args.sent, tokenizer.mask_token, eid2idx)

    pickle.dump(entity_pos, open(args.entity_pos_out, 'wb'))
    get_pretrained_emb(model, tokenizer, sentences, entity_pos, eid2idx, np_file=args.npy_out)
