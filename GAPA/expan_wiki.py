...
def pre_expan(a, kkkkk, lll):
    ...
    for i in keywords:
        ...
        if hh in glove.stoi:
            ...
            qq.append(i)
            dd.append(aaa)
    scores_1 = cos([query_sets[0]], [query_sets[1]])
    scores_2 = cos([query_sets[0]], [query_sets[2]])
    scores_3 = cos([query_sets[2]], [query_sets[1]])
    if np.mean(scores_1[:], axis=0) > 0.85:
        return a
    if np.mean(scores_2[:], axis=0) > 0.85:
        return a
    if np.mean(scores_3[:], axis=0) > 0.85:
        return a
...
def candidate(query_sets):
    ...
    for i in query_sets:
        if i in connect:
            num1 += 1
    if num1 == 3:
        num_qianduoshaoge = 525
        num_qianduoshaoge_1 = 600
        num_qianduoshaoge_2 = 50
    else:
        num_qianduoshaoge = 350
        num_qianduoshaoge_1 = 300
        num_qianduoshaoge_2 = 20
    result = []

    if len(query_sets) > 14:
        num_qianduoshaoge = 55
...
class Expan(object):
    ...
    def __init__(self, args, device, model_name='bert-large-cased-whole-word-masking', dim=1024):
        ...
        self.entity_pos = pickle.load(
            open('./wiki/entity_pos.pkl', 'rb'))

        self.pretrained_emb = np.memmap('./wiki/pretrained_emb.npy',
...
if __name__ == '__main__':
    ...
    parser.add_argument('-output', default='./wiki/results-zhengli',
    ...
    for file in os.listdir(os.path.join(args.dataset, 'query')):
        ...
        for i in range(len(query_sets)):
            ...
            expan_glove_2 = pre_expan(expan_glove_1, 0.651, 2)
